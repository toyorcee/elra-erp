...
import ELRALogo from "../../../../../assets/ElranewLogo.png";
...
const OffboardingManagement = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      console.log("🔍 Fetching offboarding lifecycle data");

      const [lifecyclesRes, statsRes] = await Promise.all([
        userModulesAPI.employeeLifecycle.getOffboarding(),
        userModulesAPI.employeeLifecycle.getStats(),
      ]);

      console.log("📊 Offboarding lifecycles response:", lifecyclesRes);
      console.log("📈 Stats response:", statsRes);

      const lifecyclesData = lifecyclesRes.data.docs || [];

      console.log("🔍 Backend filtered offboarding data:", lifecyclesData);
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border border-[var(--elra-border-primary)]">
            <div className="flex items-center space-x-4">
              <div className="p-2 bg-[var(--elra-primary)] bg-opacity-10 rounded-lg">
                <CheckCircleIcon className="w-6 h-6 text-[var(--elra-primary)]" />
              </div>
              <div>
                  ...
                  <div className="mb-6">
                    <div className="flex items-center space-x-2 mb-4">
                      ...
                      <h4 className="text-md font-semibold text-gray-900">
                        Completed Tasks (
                        {selectedUser.lifecycles.Offboarding.checklist?.filter(
                          (item) => item.isCompleted
                        ).length || 0}
                        /
                        {selectedUser.lifecycles.Offboarding.checklist
                          ?.length || 0}
                        )
                      </h4>
                    </div>
                    ...
                    <div className="space-y-3">
                      {selectedUser.lifecycles.Offboarding.checklist?.map(
                        (item, index) => (
                          <div
                            key={item._id}
