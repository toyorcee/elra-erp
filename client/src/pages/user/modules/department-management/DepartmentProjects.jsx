...
const DepartmentProjects = () => {
  ...
  const handleManageTasks = async (project) => {
    console.log("📋 [TASKS] Managing tasks for project:", project);
  ...
  const fetchProjectTasks = async (projectId) => {
    ...
    try {
      const { fetchProjectTasks: getProjectTasks } = await import(
        "../../../../services/taskAPI"
      );
  ...
  const handleUpdateTaskStatus = async (taskId, newStatus) => {
    ...
    try {
      const { updateTaskStatus } = await import("../../../../services/taskAPI");
      const response = await updateTaskStatus(taskId, newStatus);
      if (response.success) {
        toast.success(`Task ${newStatus.replace("_", " ")} successfully!`);
        await fetchProjectTasks(selectedProjectForTasks._id);
          ...
          actions={{
            ...
            customActions: (project) => (
              <div className="flex space-x-2">
                ...
                {/* Manage Tasks Button - Show for implementation status */}
                {project.status === "implementation" && (
                  <button
                    ...
                    title="Manage Implementation Tasks"
                  >
                    {loadingTasks ? (
                ...
                {/* View Certificate Button - Show for completed status */}
                {project.status === "completed" && (
                  <button
                    onClick={() => {
                      navigate(
                        `/dashboard/modules/projects/certificate/${project._id}`
                      );
                    }}
                    ...
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex space-x-3">
                        <button
                          ...
                          className="bg-blue-50 text-blue-600 px-3 py-2 rounded-lg hover:bg-blue-100 transition-colors text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
                          disabled={loadingTasks}
                        >
                          {loadingTasks ? (
                            <>
                    ...
                    {loadingTasks ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-500">
                          No implementation tasks have been created for this
                          project yet.
                        </p>
                      </div>
                    )}
